...
describe("opengraph-image source", () => {
  it("imports siteConfig and references brand fields by name", async () => {
    const src = await fs.readFile(path.join(process.cwd(), "app", "opengraph-image.tsx"), "utf8");
  ...
  it("the apple-icon and twitter-image follow the same pattern", async () => {
    const apple = await fs.readFile(path.join(process.cwd(), "app", "apple-icon.tsx"), "utf8");
    expect(apple).toContain('from "@/lib/site-config"');

    const twitter = await fs.readFile(path.join(process.cwd(), "app", "twitter-image.tsx"), "utf8");
